...
export const params = [
   { name: 'frequency',    type: 'range',  label: 'frequency',    min: 0,    max: 1, step: 0.001, default: 0.5 },
   { name: 'phase',  type: 'range',  label: 'phase',       min: 0,    max: 256,       step: 1,    default: 3 },
...
function drawPeriodic(u, v, p) {
    ...
    return (u_sine < 0 ? COLORS[2] : (v_sine < u*0.1 ? COLORS[1] : null) )
}

function drawLine(u, v, p) {

    
    let x = scale(u,0,1,0,256);
    let y = scale(v,0,1,0,256);
    let eq = Math.floor(x*p.angle)+p.offset;
    
    return ((Math.floor(y) <= (eq + p.thickness)%128 && Math.floor(y) >= (eq - p.thickness)%128 ) ? COLORS[0] : null);
}
